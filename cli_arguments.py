...
def define_argument_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument('--radar-start-delay', type=int, default=5,
                        help='delay in seconds before starting the radar after the video processing starts')
    ...
    parser.add_argument('--skip-imu', action='store_true', help='skip imu sensor processing')
    parser.add_argument('--skip-gps', action='store_true', help='skip gps sensor processing')
    ...
    parser.add_argument('--radar-config',
                        type=str,
                        default=str(current_dir / 'configuration' / 'RadarConfig.yaml'),
                        help='radar configuration file path')
    ...
    parser.add_argument('--radar-source', type=str, default=None,
                        help='path to folder to read prerecorded radar data from. Only used if "--radar-rerun" is set')
    ...
    # ------------------------------------------------------------------
    parser.add_argument('--video-config', type=str,
                        default=str(current_dir / 'configuration' / 'VideoConfig.yaml'),
                        help='video configuration file path')
    parser.add_argument('--model-weights', type=str, default=None,
                        help='model weights path. yolo wil attempt to download the weight if the arg is not a path')
    parser.add_argument('--video-source', type=str, default=None,
                        help='source of the video data, supports all yolo default options')
    parser.add_argument('--disable-save-video', action='store_true', help='disable saving the video data')
    parser.add_argument('--show-live-video', action='store_true',
                        help='if the live video feed should be shown, this requires a display')
    parser.add_argument('--video-proc-delay', type=float, default=0.0,
                        help='time to wait before the next video frame is processed')
    ...
    parser.add_argument('--tracking-config',
                        type=str,
                        default=str(current_dir / 'configuration' / 'TrackingConfig.yaml'),
                        help='tracking configuration file path')
    ...
    parser.add_argument('--batching-time', type=float, default=0.3,
                        help='time to accumulate detections before sending to tracking algorithm')
    return parser
...
def update_video_config(config: VideoConfiguration, args: argparse.Namespace) -> VideoConfiguration:
    ...
    # Begin: Added on 2025-04-07
    if args.video_config is not None:
        config.config_path = args.video_config
    # End
